...
import (
	...
	"star_mall_api/order-web/util"
	"star_mall_api/order-web/util/register/consul"
	vd "star_mall_api/order-web/validator"
	"syscall"
...
func main() {
	...
	if err := registerClient.Register(global.ServerConfig.Host, global.ServerConfig.Port, global.ServerConfig.Name, global.ServerConfig.Tags, serviceId); err != nil {
		zap.S().Panic("[order-web] 服务注册失败: ", err.Error())
	}
	...
	if err := registerClient.DeRegister(serviceId); err != nil {
		zap.S().Info("注销服务 [order-web] 失败: ", err.Error())
	} else {
		zap.S().Info("注销服务 [order-web] 成功")
	}
}
